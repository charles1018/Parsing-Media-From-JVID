...
class NetworkManager:
    ...
    def __init__(self, headers=None, timeout=None, min_request_interval=None, console=None):
        """
        初始化網路管理器
    ...
    def throttle_request(self):
        ...
        # 加入隨機成分，使請求間隔更自然
        target_interval = self.min_request_interval + random.uniform(0, self.INTERVAL_JITTER_MAX)
    ...
    def handle_response_status(self, status_code, url):
        ...
        if status_code == 429:  # Too Many Requests
            ...
            # 服務器錯誤，短時間等待後重試
            wait_time = random.uniform(self.SERVER_ERROR_WAIT_MIN, self.SERVER_ERROR_WAIT_MAX)
            if self.console:
    ...
    def request_with_retry(self, url, max_retries=None, backoff_factor=None):
        ...
        while retries < max_retries:
            try:
                ...
                # 每次請求隨機更換 User-Agent
                if 'user-agent' in self.headers and random.random() < self.UA_CHANGE_PROBABILITY:
                    self.headers['user-agent'] = self.get_random_user_agent()
