...
class ParsingMediaLogic:
    def __init__(self, obj):
        ...
        self.diagnostic_enabled = (
            obj.diagnostic_mode if hasattr(obj, "diagnostic_mode") else False
        )  # 從命令列參數獲取診斷模式設置
        self.thread_count = (
    ...
    def update_headers(self) -> dict:
        ...
        self.console.print("⚠️  未找到 cookies 文件,嘗試讀取 permissions.txt...")
        try:
            ...
            self.console.print("✓ 成功從 permissions.txt 讀取認證資訊")
            return headers
        except Exception as e:
            self.console.print(f"⚠️  無法讀取 permissions.txt: {e}")
            self.console.print("⚠️  將使用基本請求頭（部分功能可能受限）")
    ...
    def diagnostic_analyze_page(self, url, html_content):
        ...
        try:
            ...
            # 記錄頁面獲取
            self.diagnostic_mode.log_step(
                "fetch_page",
                {"url": url, "status": "success", "content_length": len(html_content)},
            )
            ...
            self.diagnostic_mode.log_step(
                "analyze_structure",
                {
                    ...
                    "media_containers_count": len(
                        analysis_result["potential_media_containers"]
                    ),
                },
            )

            # 獲取建議的提取策略
            suggestions = self.diagnostic_mode.suggest_extraction_strategies(
                analysis_result
            )
            self.diagnostic_mode.log_step(
            ...
            for method in [
                ...
                "script_variable_extraction",
            ]:
                result = self.diagnostic_mode.test_extraction_method(
                    method, html_content
                )
                ...
                if result["success"]:
                    self.console.print(
                        f"[診斷模式] 方法 '{method}' 成功提取URL: {result['url']}"
                    )

            # 如果有成功的工作示例，與當前頁面進行比較
            working_urls_file = os.path.join(
            ...
            if os.path.exists(working_urls_file):
                ...
                if working_urls:
                    comparison = self.diagnostic_mode.compare_with_working_examples(
                        working_urls, html_content
                    )
    ...
    def identify_type_operation(self):
        ...
        # 使用內容檢測器判斷頁面類型
        has_video, has_image = self.content_detector.detect_content_types(
            res.text, soup
        )
        ...
        if has_video:
            # 提取影片URL
            video_urls, video_types = self.content_detector.extract_video_urls(
                res.text, soup
            )
            ...
            if self.diagnostic_enabled and self.diagnostic_mode:
                # 嘗試直接正則表達式
                regex_result = self.diagnostic_mode.test_extraction_method(
                    "direct_regex_m3u8", res.text
                )
                if regex_result["success"] and regex_result["url"] not in video_urls:
                    ...
                    self.console.print(
                        f"[診斷模式] 使用正則表達式成功提取URL: {regex_result['url']}"
                    )

                # 嘗試JSON數據提取
                json_result = self.diagnostic_mode.test_extraction_method(
                    "json_data_extraction", res.text
                )
                if json_result["success"] and json_result["url"] not in video_urls:
                    ...
                    self.console.print(
                        f"[診斷模式] 從JSON數據成功提取URL: {json_result['url']}"
                    )

                # 嘗試腳本變量提取
                script_result = self.diagnostic_mode.test_extraction_method(
                    "script_variable_extraction", res.text
                )
                if script_result["success"] and script_result["url"] not in video_urls:
                    ...
                    self.console.print(
                        f"[診斷模式] 從腳本變量成功提取URL: {script_result['url']}"
                    )
            ...
            if not video_urls:
                ...
                video_processor = VideoProcessor(
                    self.network_manager, self.path, self.console
                )
                # 設定執行緒數量
        ...
        if has_image:
            ...
            image_processor = ImageProcessor(
                self.network_manager, image_path, self.console
            )
            # 設定執行緒數量
