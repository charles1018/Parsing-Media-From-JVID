...
class ProgressManager:
    ...
    def load_progress(self, url, type_name='auto'):
        ...
        try:
            ...
            self.console.print(f"已從 {save_time} 的進度恢復，將繼續下載剩餘 {len(remaining_todo_list)} 個檔案")
        ...
        try:
            ...
            if progress_data.get('url') == url:
                ...
                if remaining_count > 0:
                    save_time = progress_data.get('timestamp', '未知時間')
                    self.console.print(f"發現在 {save_time} 的未完成下載，剩餘 {remaining_count} 個檔案")
                    ...
                    if auto_resume:
                        ...
                        # 詢問使用者是否繼續下載
                        resume_choice = str(input('是否繼續上次的下載? (y/n): ')).lower().strip()
