...
class DiagnosticMode:
    ...
    def start_session(self, url: str) -> None:
        ...
        self.current_session_id = f"{int(time.time())}_{urlparse(url).path.split('/')[-1]}"
        self.diagnostic_data = {
            "url": url,
    ...
    def analyze_page_structure(self, html_content: str) -> Dict[str, Any]:
        ...
        for script in soup.find_all('script'):
            if script.string:
                ...
                if any(keyword in script_content for keyword in ['video', 'm3u8', 'mp4', 'transcode', 'playList']):
                    analysis_result["scripts"].append({
                        "type": script.get('type', 'text/javascript'),
                        "content_preview": script_content[:500] + "..." if len(script_content) > 500 else script_content
                    })
        ...
        for div in soup.find_all('div'):
            div_str = str(div)
            if any(keyword in div_str for keyword in ['video', 'm3u8', 'mp4', 'transcode', 'playList']):
                container = {
                    "id": div.get('id', 'unknown'),
                    "class": div.get('class', []),
                    "attributes": {k:v for k,v in div.attrs.items() if k not in ['id', 'class']},
                    "content_preview": div_str[:500] + "..." if len(div_str) > 500 else div_str
                }
                ...
                if response.status_code == 200:
                    ...
                    for pattern_type in working_patterns:
                        if pattern_type in current_patterns:
                            if working_patterns[pattern_type] and not current_patterns[pattern_type]:
                                comparison_result["differences_found"] = True
                                comparison_result["pattern_differences"][pattern_type] = {
                                    "working_example": working_patterns[pattern_type],
                                    "current_page": "無匹配模式"
                                }
                            elif set(working_patterns[pattern_type]) != set(current_patterns[pattern_type]):
                                comparison_result["differences_found"] = True
                                comparison_result["pattern_differences"][pattern_type] = {
                                    "working_example": working_patterns[pattern_type],
    ...
    def test_extraction_method(self, method_name: str, html_content: str) -> Dict[str, Any]:
        """
        測試特定的提取方法
        ...
        if method_name == "meta_transcode":
            ...
            if len(content_check) != 0:
                m3u8_url = content_check[0].split('vidSrc=')[-1].split('.m3u8')[0] + '.m3u8'
                result["success"] = True
                result["url"] = m3u8_url
            ...
            for json_str in json_matches:
                try:
                    # 嘗試修復和解析JSON
                    fixed_json = re.sub(r'([{,])\s*([a-zA-Z0-9_]+):', r'\1"\2":', json_str)
                    data = json.loads(fixed_json)
                    
                    # 在數據中尋找URL
                    for key, value in data.items():
                        if isinstance(value, str) and (value.endswith('.m3u8') or value.endswith('.mp4')):
                            result["success"] = True
                            result["url"] = value
            ...
            # 方法5: 從腳本變量中提取URL
            script_pattern = re.compile(r'var\s+(\w+)\s*=\s*[\'"]([^\'"]*(\.m3u8|\.mp4))[\'"]')
    ...
    def finalize_session(self, success: bool, final_url: Optional[str] = None, 
        ...
        # 保存診斷數據
        save_path = os.path.join(self.base_path, f"diagnostic_{self.current_session_id}.json")
