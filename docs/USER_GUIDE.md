...
3. [Web UI 使用指南](#web-ui-使用指南)（推薦新手）
4. [命令列使用](#命令列使用)
5. [進階功能](#進階功能)
6. [常見問題](#常見問題)
7. [最佳實踐](#最佳實踐)
...
> 🌟 **推薦新手使用** - 如果你不熟悉命令列操作，Web UI 是最簡單的使用方式！
...
Web UI 提供直覺的圖形化介面，包含以下功能：

| 元件 | 說明 |
|------|------|
| **JVID 網址** | 貼上要下載的 JVID 頁面網址 |
| **儲存路徑** | 檔案儲存位置（預設為 `media`） |
| **執行緒數** | 下載執行緒數量（1-8，建議 1-3） |
| **自動續傳** | 如有未完成的下載，自動繼續 |
| **開始下載** | 點擊開始下載 |
| **清除** | 清除所有輸入欄位 |
| **關閉伺服器** | 結束 Web UI 程式 |
...
2. **輸入網址**
   - 在瀏覽器開啟的頁面中，找到「JVID 網址」欄位
   - 貼上 JVID 頁面網址，例如：`https://www.jvid.com/v/12345`
...
- 確保 Cookie 文件已放置在專案根目錄
- 下載過程中請勿關閉瀏覽器頁面
- 如需停止下載，可直接點擊「關閉伺服器」按鈕
- Web UI 預設監聽 0.0.0.0:7860，可從區域網路內其他裝置訪問

---

## 命令列使用

> 💡 如果你熟悉命令列操作，命令列方式提供更多彈性和進階功能。
...
> ⚠️ **注意**: 此功能目前為實驗性功能，正在進行架構重構中。目前的行為是重新開始下載而非從中斷點恢復。

當下載中斷時，使用 `-a` 參數啟用自動續傳模式：
...
uv run jvid-webui
```
程式會自動開啟瀏覽器訪問 http://localhost:7860

#### Q: Web UI 無法開啟瀏覽器

**A:** 嘗試以下方法：
1. 手動在瀏覽器輸入 `http://localhost:7860`
2. 如果 7860 埠被佔用，請關閉佔用的程式後重試
3. 檢查防火牆設定

#### Q: Web UI 如何關閉？

**A:** 有兩種方式：
1. 點擊頁面底部的「⏹️ 關閉伺服器」按鈕
2. 在終端機按 `Ctrl+C` 中斷程式

#### Q: Web UI 和命令列有什麼差異？

**A:** 功能相同，Web UI 提供圖形介面更易上手。命令列支援更多進階參數（如診斷模式 `-d`）。建議新手使用 Web UI，熟練後可改用命令列。
